...
class DoubleODVGDataset(VisionDataset):
    """
    Args:
        root (string): Root directory where images are downloaded to.
        anno (string): Path to json annotation file.
    ...
    def __init__(
        ...
        self.styles = glob.glob(os.path.join(style_root, "*.jpg"))
        ...
        self.style_transforms = T.Compose([
    ...
    def __getitem__(self, index: int):
        ...
        style_image = Image.open(random.choice(self.styles)).convert('RGB')
        w, h = image.size
        anno = meta["detection"]
        instances = [obj for obj in anno["instances"]]
        boxes = [obj["bbox"] for obj in instances]
        ...
        if self.transforms is not None:
            ...
            style_image, _ = self.style_transforms(style_image, None)

        return image, target, style_image
...
def build_doubleodvg(image_set, args, datasetinfo):
    img_folder = datasetinfo["root"]
    ann_file = datasetinfo["anno"]
    label_map = datasetinfo["label_map"] if "label_map" in datasetinfo else None
    try:
        ...
        strong_aug = False
    print(img_folder, ann_file, label_map)
    dataset = DoubleODVGDataset(img_folder, ann_file, args.style_root, label_map, max_labels=args.max_labels,
